...
package eu.cdevreeze.tryjava.trytrees.mutable.xml.convert;

import eu.cdevreeze.tryjava.trytrees.mutable.xml.model.ElemNode;
import eu.cdevreeze.tryjava.trytrees.mutable.xml.model.TextNode;
import eu.cdevreeze.tryjava.trytrees.mutable.xml.model.XmlNode;
...
public class SaxonConverter implements XmlNodeConverter<XdmNode> {
    ...
    private ElemNode convertSaxonElemToXmlNode(XdmNode node) {
        ...
        var attributes = node.axisIterator(Axis.ATTRIBUTE).stream().collect(Collectors.toMap(this::getAttributeName, this::getAttributeValue));

        // Recursion
        var children = node.axisIterator(Axis.CHILD).stream().filter(this::isSupportedNode).map(this::convertToXmlNode).toList();
