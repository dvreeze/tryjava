...
/**
 * "Step finder" for a hidden triplet in a column.
 * See <a href="https://www.learn-sudoku.com/hidden-triplets.html">hidden-triplet</a>.
 *
 * @author Chris de Vreeze
 */
public record HiddenTripletInColumn(GridApi startGrid, int columnIndex) implements StepFinderInGivenHouse {
    ...
    private Optional<StepResult> findNextStepResult(HiddenTriplet hiddenTriplet, ImmutableMap<Position, ImmutableSet<Integer>> candidates, PencilMarks pencilMarks) {
        ...
        return optCandidateToFillIn.map(candidateToFillIn -> new SetCellValueStep(
                        ...
                        "Filling cell in column after processing hidden triplet"
                ...
                .or(() -> (!adaptedPencilMarks.limits(pencilMarks)) ? Optional.empty() :
                        Optional.of(new UpdatePencilMarksStep(
                                "Updating pencil marks in column after processing hidden triplet",
                                adaptedPencilMarks
